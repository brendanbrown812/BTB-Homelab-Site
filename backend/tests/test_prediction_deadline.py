...
from app.features.predictions.routes import _default_lock_at, _migrate_open_lock_to_thursday_seven
...
class PredictionDeadlineTests(unittest.TestCase):
    ...
    def test_temporary_late_thursday_deadline_moves_back_to_seven(self):
        week = PredictionWeek(
        ...
        changed = _migrate_open_lock_to_thursday_seven(week)

        self.assertTrue(changed)
        self.assertEqual(
            week.lock_at.astimezone(CENTRAL_TIME),
            datetime(2026, 9, 17, 19, tzinfo=CENTRAL_TIME),
        )
    ...
    def test_existing_thursday_seven_deadline_is_unchanged(self):
        week = PredictionWeek(
            season_id=uuid.uuid4(),
            week_number=2,
