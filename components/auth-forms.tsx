...
export function LoginForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const body = await response.json() as { access_token: string }; localStorage.setItem("btb_access_token", body.access_token); router.push("/predictions");
...
export function SetupForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) { const body = await response.json() as { detail?: string }; throw new Error(body.detail ?? "Account setup failed."); }
      router.push("/login");
