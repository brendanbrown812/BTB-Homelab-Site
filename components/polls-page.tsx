...
function PollFormDialog({ poll, onSaved }: { poll?: Poll; onSaved: (poll: Poll) => void }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setSaving(true); setError("");
    try {
      const savedPoll = await apiFetch<Poll>(poll ? `/polls/${poll.id}` : "/polls", { method: poll ? "PUT" : "POST", body: JSON.stringify({ question: form.get("question"), description: form.get("description") || null, selection_mode: form.get("selection_mode"), options: options.map(option => option.text), closes_at: closesAt ? new Date(closesAt).toISOString() : null }) });
