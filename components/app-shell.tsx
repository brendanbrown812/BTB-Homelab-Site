...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const [open, setOpen] = useState(false); const [user, setUser] = useState<CurrentUser | null>(null); const [canSubmitWriteup, setCanSubmitWriteup] = useState(false);
  const [predictionsExpanded, setPredictionsExpanded] = useState(true); const [ptgotwExpanded, setPtgotwExpanded] = useState(true);
  useEffect(() => {
    if (!getToken()) { router.replace("/login"); return; }
    apiFetch<CurrentUser>("/auth/me").then(value => {
      setUser(value);
      if (value.role === "admin") setCanSubmitWriteup(true);
      else apiFetch<Array<{ id: string }>>("/ptgotw/editable").then(items => setCanSubmitWriteup(items.length > 0)).catch(() => setCanSubmitWriteup(false));
  ...
  const itemClass = (href: string) => `flex items-center gap-3 rounded-xl px-3 py-2.5 text-[15px] transition ${path === href || (href === "/ptgotw" && path.startsWith("/ptgotw/") && path !== "/ptgotw/submit") ? "bg-primary font-semibold text-primary-foreground" : "text-slate-400 hover:bg-white/5 hover:text-white"}`;
  const nav = <>
