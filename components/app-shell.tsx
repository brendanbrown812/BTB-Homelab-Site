...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="min-h-screen lg:grid lg:grid-cols-[245px_1fr]">
    <aside className="fixed inset-y-0 left-0 z-30 hidden w-[245px] flex-col border-r border-white/8 bg-[#0b1019]/95 px-4 lg:flex"><Brand /><nav className="no-scrollbar flex-1 overflow-y-auto pb-4">{nav}</nav><Account user={user} onUpdated={setUser} onLogout={logout} /></aside>
    {open && <div className="fixed inset-0 z-40 bg-black/70 backdrop-blur-sm lg:hidden" onClick={() => setOpen(false)}><aside className="flex h-full w-[280px] flex-col border-r border-white/10 bg-[#0b1019] px-4" onClick={e => e.stopPropagation()}><div className="flex items-center justify-between"><Brand /><button aria-label="Close navigation" onClick={() => setOpen(false)}><X /></button></div><nav className="no-scrollbar flex-1 overflow-y-auto pb-4">{nav}</nav><Account user={user} onUpdated={setUser} onLogout={logout} /></aside></div>}
