...
export function PTGWriteupEditor() {
  ...
  const [selectedId, setSelectedId] = useState("new"); const [year, setYear] = useState(new Date().getFullYear()); const [week, setWeek] = useState(1); const [authorId, setAuthorId] = useState(""); const [submittedByAuthor, setSubmittedByAuthor] = useState(false); const [dueDate, setDueDate] = useState(""); const [content, setContent] = useState("");
  ...
  useEffect(() => {
    Promise.all([apiFetch<CurrentUser>("/auth/me"), apiFetch<PTGWriteup[]>("/ptgotw/editable")]).then(async ([user, items]) => {
      ...
      const requestedId = new URLSearchParams(window.location.search).get("id"); const requested = items.find(item => item.id === requestedId);
      if (user.role === "admin") {
        const choices = await apiFetch<Author[]>("/ptgotw/authors");
        setAuthors(choices);
        if (requested) { setSelectedId(requested.id); setYear(requested.year); setWeek(requested.week); setAuthorId(requested.author.id); setSubmittedByAuthor(requested.submitted_by_author ?? false); setDueDate(requested.due_date ?? ""); setContent(requested.content_html); }
        else setAuthorId(choices[0]?.id ?? "");
      } else if (items[0]) {
        const first = requested ?? items[0]; setSelectedId(first.id); setYear(first.year); setWeek(first.week); setAuthorId(first.author.id); setSubmittedByAuthor(true); setDueDate(first.due_date ?? ""); setContent(first.content_html);
      }
  ...
  function selectWriteup(writeup: PTGWriteup, source = editable) {
    setSelectedId(writeup.id); setYear(writeup.year); setWeek(writeup.week); setAuthorId(writeup.author.id); setSubmittedByAuthor(writeup.submitted_by_author ?? true); setDueDate(writeup.due_date ?? ""); setContent(writeup.content_html); setNotice(""); setError("");
    if (!source.some(item => item.id === writeup.id)) setEditable(items => [writeup, ...items]);
  }
  function changeSelection(value: string) {
    if (value === "new") { setSelectedId("new"); setYear(new Date().getFullYear()); setWeek(1); setAuthorId(authors[0]?.id ?? ""); setSubmittedByAuthor(false); setDueDate(""); setContent(""); setNotice(""); return; }
  ...
  async function save(isPublished: boolean) {
    ...
    const metadata = me.role === "admin" ? { year, week, author_id: authorId, submitted_by_author: submittedByAuthor, due_date: submittedByAuthor ? dueDate || null : null } : {};
    try {
      ...
      selectWriteup(result); setEditable(items => [result, ...items.filter(item => item.id !== result.id)]); window.dispatchEvent(new Event("ptgotw-updated")); setNotice(isPublished ? "Writeup published to the archive." : me.role === "admin" && submittedByAuthor ? "Draft saved. The author can begin working on it immediately." : "Draft saved. Only you and commissioners can see it.");
  ...
  const selectedWriteup = editable.find(item => item.id === selectedId);
  const daysUntilDue = dueDate ? calendarDaysUntil(dueDate) : null;
  const authorCanPublish = me.role === "admin" || !!selectedWriteup?.is_published || (daysUntilDue !== null && daysUntilDue <= 7 && daysUntilDue >= -7);
  return <>
    ...
    <form onSubmit={event => { event.preventDefault(); void save(true); }} className="grid gap-5 xl:grid-cols-[300px_minmax(0,1fr)]">
      <aside className="h-fit space-y-5 rounded-2xl border border-white/8 bg-card p-5">
        ...
        {submittedByAuthor && <div className="space-y-2"><Label htmlFor="writeup-due-date">Due date</Label>{me.role === "admin" ? <><Input id="writeup-due-date" type="date" value={dueDate} onChange={event => setDueDate(event.target.value)} required /><p className="text-xs leading-relaxed text-slate-500">The author can edit immediately, publish starting 7 days before, and make changes through 7 days after this date.</p></> : <><p className="rounded-lg border border-white/8 bg-white/[.025] px-3 py-2 text-sm text-slate-300">{dueDate ? new Date(`${dueDate}T12:00:00`).toLocaleDateString(undefined, { weekday: "long", month: "long", day: "numeric", year: "numeric" }) : "No due date"}</p><p className="text-xs leading-relaxed text-slate-500">You can edit now and through 7 days after the due date. Publishing opens 7 days before it is due.</p></>}</div>}
      </aside>
      <section className="overflow-hidden rounded-2xl border border-white/8 bg-card"><RichTextEditor value={content} onChange={setContent} /><div className="flex flex-col gap-3 border-t border-white/8 px-5 py-4 sm:flex-row sm:items-center sm:justify-between"><div>{error && <p role="alert" className="text-sm text-red-300">{error}</p>}{notice && <p role="status" className="text-sm text-emerald-300">{notice}</p>}{overLimit && <p role="alert" className="text-sm text-red-300">Shorten the writeup to 30,000 characters before saving.</p>}{me.role !== "admin" && !authorCanPublish && daysUntilDue !== null && <p className="text-sm text-amber-300">Publishing opens in {daysUntilDue - 7} {daysUntilDue - 7 === 1 ? "day" : "days"}. You can keep saving drafts now.</p>}</div><div className="flex flex-col gap-2 sm:flex-row"><Button type="button" variant="secondary" disabled={saving || !authorId || overLimit || (me.role === "admin" && submittedByAuthor && !dueDate)} onClick={() => void save(false)}><Save className="h-4 w-4" />{selectedWriteup?.is_published ? "Move to draft" : "Save draft"}</Button><Button type="submit" disabled={saving || !authorId || !plainText(content).trim() || overLimit || (me.role === "admin" && submittedByAuthor && !dueDate) || !authorCanPublish} className="bg-primary font-bold text-primary-foreground">{saving ? "Saving…" : selectedWriteup?.is_published ? "Save changes" : "Publish writeup"}</Button></div></div></section>
    </form>
  </>;
}
